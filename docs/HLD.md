...
* **Naman Goyal (Core Developer):** Designed the multi-layer framework architecture. Developed the switch energy dissipation and regional carbon accounting models. Built the FastAPI backend routing endpoints, application schemas, and coordinated subsystem integration.
...
* **Ranjit Mohanty (Data Analyst & Research):** Researched real-world regional power grid carbon intensity datasets. Designed the experimental scenario matrix and structured automated benchmarking routines and presentation deck.
* **Atharva Sharma (QA & Technical Documentation):** Authored the High-Level Design document. Executed integration test verification across the API and simulation pipelines and formulated the composite cost function and normalization engine in Python.
